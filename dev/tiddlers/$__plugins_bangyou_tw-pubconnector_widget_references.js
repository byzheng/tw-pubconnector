...
(function () {
    ...
    */
    ReferencesWidget.prototype.render = function (parent, nextSibling) {
        ...
        if (!tiddlerTitle) {
            containerDom.innerHTML = "ReferencesWidget: No tiddler title provided";
            return;
        }

        // Check if the tiddler exists
        if (!$tw.wiki.tiddlerExists(tiddlerTitle)) {
            containerDom.innerHTML = "ReferencesWidget: Tiddler '" + tiddlerTitle + "' does not exist";
            return;
        }
        ...
        if (!doi || typeof doi !== "string" || !/^10\.\d{4,9}\/[-._;()/:A-Z0-9]+$/i.test(doi)) {
            containerDom.innerHTML = "ReferencesWidget: Tiddler '" + tiddlerTitle + "' does not have a valid bibtex-doi field";
            return;
        }

        fetch(`/literature/references/${encodeURIComponent(doi)}`)
            .then(response => {
                if (!response.ok) {
    ...
    exports.references = ReferencesWidget;
